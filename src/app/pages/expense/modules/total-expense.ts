export class TotalExpense {
  ...
  getIncrementalExpenses(month: string, totalExpenses: { date: string, amount: number }[]): { date: string, amount: number }[] {
    const expenses:any[] = [];
    const monthDate = new Date(month);

    // Loop through each day of the month
    for (let i = 1; i <= new Date(monthDate.getFullYear(), monthDate.getMonth() + 1, 0).getDate(); i++) {
      const date = new Date(monthDate.getFullYear(), monthDate.getMonth(), i).toISOString().substr(0, 10);
      let total = 0;
